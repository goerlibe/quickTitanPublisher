...
export class QuickMqttPublisher {
  ...
  public host = 'mqtt://localhost';
  public prefix = 'reajet/testing';
  public client = 'Quick Titan-Micro-MQTT';
  public uuid = 42;
  public topic = '';
  public body: string;
  public quickOptions: QuickOption[] = [
    ...
    },
    {
      desc: 'set: queue of 3',
      topic: 'cmd/print/job/set',
      body: [['first.job', 1], ['second.job', 1], ['third.job', 1]]
    },
    {
  ...
  public update(data: { host?; prefix?; client?; uuid?; topic?; body: string; quickPub? }): void {
    if (data.host) {
      this.host = data.host;
    }
    if (data.prefix) {
    ...
    if (data.quickPub) {
      if (data.quickPub !== this.quickOptions[0].desc) {
        const option = this.quickOptions.find(opt => opt.desc === data.quickPub);
